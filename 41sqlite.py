...
sql = f' select JOB_ID, count(FIRST_NAME) cnt from emp '\
      f' group by JOB_ID order by cnt desc '
cur.execute(sql)
...
medal = 'Gold'
sql = f' select Country, count(medal) cnt from medals '\
      f' where medal = "{medal}" group by Country '\
