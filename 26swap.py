...
def swap2(nums):
    ...
    nums[1] = c
    print('swap 내부',
          nums[0], nums[1], id(nums[0]), id(nums[1]))
...
# 함수내에서도 조작할 수 있음
# 단, 외부 변수에 종속적인 함수는 그다지 좋은 함수가 아님

# ex) 숫자 바뀌기 3
# 입력받은 매개변수의 값을 서로 바꿔 출력하는 함수 - swap3
# 매개변수를 불변객체로 정의하고 global이라는 키워드 사용 (비추!)
